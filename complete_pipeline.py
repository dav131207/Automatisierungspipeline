...
def befuelle_interne_kennung(page, nummer: str, interne_kennung: str):
    ...
    try:
        # Wert aus Spalte T direkt eintragen (Recording: kein Tab-Wechsel nötig)
        page.locator(f"#TN_Intern{n}").fill(interne_kennung)
...
def speichern_und_warten(page, label: str = ""):
    """Speichert und wartet bis die Seite wieder stabil ist."""
    try:
        page.get_by_role("button", name="Speichern").click()
        page.wait_for_load_state("networkidle", timeout=15000)
        # Warte bis kein Spinner mehr sichtbar ist
        try:
...
def befuelle_hauptformular(page, daten: dict, nummer: str):
    ...
    # ── Tab öffnen ─────────────────────────────────────────────────────────────
    log.info("    Öffne Tab Allg. Informationen...")
    try:
        klicke_tab(page, "Allg. Informationen")
    except Exception as e:
        log.warning("    ⚠️   Tab Allg. Informationen nicht gefunden: %s", e)
    ...
    # Telefon (auch ohne Adresse eintragbar)
    tel = daten.get("Telefon", "")
    if tel:
        try:
            loc_t = page.locator(f"#TN_Tel{n}")
            loc_t.wait_for(state="visible", timeout=6000)
            loc_t.fill(tel)
            log.info("    ✅  %-32s = %s", "Telefonnummer", tel)
        except Exception as e:
            try:
                page.locator('input[name="TN_Tel"]').fill(tel)
                log.info("    ✅  Telefon (Fallback) = %s", tel)
    ...
    speichern_und_warten(page, "Allg. Informationen")
    ss(page, f"05_metadaten_fertig_{n}")
    log.info("    ✅  Allg. Informationen vollständig gespeichert.")
...
def befuelle_eintrittsdaten(page, daten: dict, nummer: str):
    ...
    """
    n = nummer
    log.info("    Öffne Tab Eintritts-Daten...")
    try:
        klicke_tab(page, "Eintritts-Daten")
    except Exception as e:
        log.warning("    ⚠️   Tab Eintritts-Daten: %s", e)
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        for zeile in zeilen:
            ...
            # ── 9. Interne Kennung berechnen → Metadaten-Tab nachtragen ──────
